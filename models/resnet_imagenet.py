...
class Bottleneck(nn.Module):
    ...
    def __init__(self, inplanes, planes, cfg, stride=1, downsample=None):
        ...
        self.conv3 = nn.Conv2d(cfg, planes * self.expansion, kernel_size=1, bias=False)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers:list, cfg=None, num_classes=1000):
        self.inplanes = 64
    ...
    def _make_layer(self, block, planes, blocks, cfg, stride=1):
        downsample = None
        # print('self.inplanes:{}, planes:{}, plane*ex:{}'.format(self.inplanes, planes, planes * block.expansion))
        ...
        layers = []
        # print(self.inplanes, planes, cfg[0], stride)
...
def resnet18():
    return ResNet(BasicBlock, layers=[2,2,2,2])

def resnet34():
    return ResNet(BasicBlock, layers=[3,4,6,3])

def resnet50():
    return ResNet(Bottleneck, layers=[3,4,6,3])

def resnet101():
    return ResNet(Bottleneck, layers=[3,4,23,3])

def resnet152():
    return ResNet(Bottleneck, layers=[3,8,36,3])

def resnet(arch, cfg=None):
    if arch == 18:
        return ResNet(BasicBlock, layers=[2,2,2,2], cfg=cfg)
    elif arch == 34:
        return ResNet(BasicBlock, layers=[3,4,6,3], cfg=cfg)
    elif arch == 50:
        return ResNet(Bottleneck, layers=[3,4,6,3], cfg=cfg)
    elif arch == 101:
        return ResNet(Bottleneck, layers=[3,4,23,3], cfg=cfg)
    elif arch == 152:
        return ResNet(Bottleneck, layers=[3,8,36,3], cfg=cfg)
    else:
        print('no such arch in options')
