...
parser.add_argument('--arch', type=str, default='vgg', help='input network architecture')
...
# log
log = open(os.path.join(args.save, '{}_{}_scratch.txt'.format(args.log_name, args.seed)), 'w')
...
class DE:
    def __init__(self, Dim:int, NP:int, alpha:float, cfg:list, T:int, prune_step:list, output_shape:list, finetune_or_retrain:str, param_prune_rate:float=0.0, FLOPs_prune_rate:float=0.0, F:float=0.5, CR:float=0.8):
        ...
        for i in range(len(cfg)):
            if cfg[i] == 'M':
                ...
                self.cfg_onlynum.append(cfg[i])
                self.totalchannels += cfg[i]

        for i in range(len(self.cfg_onlynum)):
            upper = int(int(self.cfg_onlynum[i] * self.alpha) / self.prune_step[i]) * self.prune_step[i]
        ...
        self.remain_flops = self.cal_flops(self.cfg_onlynum) * self.FLOPs_prune_rate
        ...
        assert len(self.cfg_onlynum) == len(self.prune_step), 'len(self.cfg_onlynum) != len(self.step)'
        assert len(self.output_shape) == len(self.prune_step), 'len(self.output_shape) != len(self.prune_step)'

        print('flops: ', self.cal_flops(self.cfg_onlynum), 'G')
        net = vgg(dataset='cifar10', cfg=cfg)
        net = net.cuda()

    def cal_flops(self, cfg):
        assert len(cfg) == len(self.cfg_onlynum)
        flops = []
        for i in range(len(cfg)):
            if i == 0:
                in_channel = 3
    ...
    def trans_cfg(self, value:np.array):
        cfg_pruned = []
        M_loc = [2, 5, 9, 13]
        ...
        for i in range(len(self.cfg)):
            if i in M_loc:
                ...
                cfg_pruned.append(int(v[i]))
        return cfg_pruned

    def copynet_byL1(self, model, value:np.array):
        value = np.array(value).astype(np.int32)
        cfg_pruned = self.trans_cfg(value)
        newmodel = vgg(dataset='cifar10', cfg=cfg_pruned)
        count = 0
        ...
        for [m0, m1] in zip(model.modules(), newmodel.modules()):
            if isinstance(m0, nn.BatchNorm2d):
                ...
                arg_max_rev = arg_max[::-1][:value[count]]
    ...
    def Boundary(self, mut:np.array):
        ...
        current_channels = np.sum(mut_new)
    ...
    def run_DE(self):
        ...
        while(t < self.T):
            ...
            p = list(self.best_unit.gene / np.array(self.cfg_onlynum))
            p = [round(i, 2) for i in p]
            print_log('flops: {} / {} = {}, channels: {} / {} = {}'.format(
                self.cal_flops(self.best_unit.gene), 
                self.cal_flops(self.cfg_onlynum),
                self.cal_flops(self.best_unit.gene) / self.cal_flops(self.cfg_onlynum),
                np.sum(self.best_unit.gene), 
                np.sum(self.cfg_onlynum),
                np.sum(self.best_unit.gene) / np.sum(self.cfg_onlynum)
            ), log)
            print_log('best choice: {}'.format(self.best_unit.gene), log)
            print_log('\t{}\n'.format(p), log)
            t += 1

        v = list(self.best_unit.gene)
        # # finetune
        ...
        # retrain
            cfg_pruned = self.trans_cfg(v)
            net_pruned = vgg(dataset='cifar10', cfg=cfg_pruned)
...
if __name__ == "__main__":
    ...
    if not args.pretrained:
        ...
        for epoch in range(args.epochs):
            ...
            save_checkpoint({
                ...
                'cfg':net.cfg
            }, is_best, filepath=args.save, save_name='vgg_best')
    ...
    step = [8, 8, 8, 8, 16, 16, 16, 32, 32, 32, 32, 32, 32]
    output_shape = [32, 32, 16, 16, 8, 8, 8, 4, 4, 4, 2, 2, 2]
    de1 = DE(
        Dim=13, 
        NP=10, 
        alpha=1, 
